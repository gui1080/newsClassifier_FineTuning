...
if etl:
    ...
    # Load the CSV file
    df_gpt4 = pd.read_csv("../DATASETS_NOTICIAS/GPT4o_Finetuning/noticias_gpt4_n3000.csv")
    df_gpt4 = df_gpt4.sample(n=2500, replace=False)

    df_original = pd.read_csv("../DATASETS_NOTICIAS/G1_Metropoles_MinhaExtracaoFinal/dataset_noticias_final.csv")
    df_original = df_original.sample(n=2500, replace=False)
...
training_args_baseline = TrainingArguments(
    output_dir="./results_fine_tune_gpt40_original_baseline",
    ...
    weight_decay=0.01,
    save_total_limit=2,
    logging_dir="./gpt4_finetuned_logs_baseline",
    warmup_steps=500,
)
...
if Train1 == True:
    ...
    trainer1.save_model("./gpt4_fine_tuned_bert_multilingual_params_base")
    trainer1.push_to_hub("gpt4_fine_tuned_bert_multilingual_params_base")

    results1 = trainer1.evaluate()

    print(results1)

    with open("logfile_params_base_gpt40_finetune.txt", "w") as file:  # Use "a" to append instead of "w" to overwrite
...
if Train2 == True:
    ...
    training_args_aggressive = TrainingArguments(
        output_dir="./results_fine_tune_gpt40_original_aggressive",
        evaluation_strategy="steps",
        eval_steps=500,  # Evaluate every 500 steps
        ...
        weight_decay=0.01,
        save_total_limit=2,
        logging_dir="./gpt4_finetuned_logs_aggressive",
    ...
    trainer2.save_model("./gpt4_fine_tuned_bert_multilingual_params_aggressive")
    trainer2.push_to_hub("gpt4_fine_tuned_bert_multilingual_params_aggressive")

    results2 = trainer2.evaluate()

    print(results2)

    with open("logfile_params_aggressive_gpt40_finetune.txt", "w") as file:  # Use "a" to append instead of "w" to overwrite
...
if Train3 == True:
    ...
    training_args_careful = TrainingArguments(
        output_dir="./results_fine_tune_gpt40_original_careful",
        evaluation_strategy="steps",
        eval_steps=500,  # Evaluate every 500 steps
        ...
        save_total_limit=3,
        logging_dir="./gpt4_finetuned_logs_careful",
        warmup_steps=500,
    )
    ...
    trainer3.save_model("./gpt4_fine_tuned_bert_multilingual_params_careful")
    trainer3.push_to_hub("gpt4_fine_tuned_bert_multilingual_params_careful")

    results3 = trainer3.evaluate()

    print(results3)

    with open("logfile_params_careful_gpt40_finetune.txt", "w") as file:  # Use "a" to append instead of "w" to overwrite
